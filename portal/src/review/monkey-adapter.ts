import type { LearningProfile } from "../../../shared/learning/core.mjs";
import { BrowserLearningProfileStore } from "../../../shared/learning/browser-store.mjs";
import {
  buildMonkeyReviewDataset,
  type MonkeyReviewDataset,
} from "../../../shared/learning/monkey-review-dataset.mjs";
import type { ReviewPlan } from "../../../shared/learning/review-session.mjs";

const PENDING_KEY = "typingGamePendingMonkeyReviewV1";

function requestId(): string {
  return `review-${Date.now().toString(36)}-${Math.random()
    .toString(36)
    .slice(2, 9)}`;
}

export async function queueMonkeyReview(
  plan: ReviewPlan,
  store = new BrowserLearningProfileStore(),
): Promise<MonkeyReviewDataset> {
  const profile: LearningProfile = await store.load();
  const dataset = buildMonkeyReviewDataset(plan, profile, requestId());
  sessionStorage.setItem(PENDING_KEY, JSON.stringify(dataset));
  return dataset;
}

export function readPendingMonkeyReview(): MonkeyReviewDataset | null {
  const raw = sessionStorage.getItem(PENDING_KEY);
  if (raw === null) return null;

  try {
    const value = JSON.parse(raw) as Partial<MonkeyReviewDataset>;
    if (
      value.version !== 1 ||
      value.type !== "typing-game:learning:v1:review-dataset" ||
      typeof value.requestId !== "string" ||
      !Array.isArray(value.items)
    ) {
      return null;
    }
    return value as MonkeyReviewDataset;
  } catch {
    return null;
  }
}

export function clearPendingMonkeyReview(requestId?: string): void {
  if (requestId !== undefined) {
    const pending = readPendingMonkeyReview();
    if (pending !== null && pending.requestId !== requestId) return;
  }
  sessionStorage.removeItem(PENDING_KEY);
}

export function postPendingMonkeyReview(
  frame: HTMLIFrameElement,
  appUrl: string,
): MonkeyReviewDataset | null {
  const pending = readPendingMonkeyReview();
  const frameWindow = frame.contentWindow;
  if (pending === null || frameWindow === null) return null;

  frameWindow.postMessage(pending, new URL(appUrl).origin);
  return pending;
}
